...
int main(void)
{
  ...
  //  CvMemStorage *houghStorage = cvCreateMemStorage(0); // *** delete by memory_leak2_20141024.patch
  ...
  while (1)  
    {
      ...
      //     return -1;
      //   }
      CvMemStorage *houghStorage = cvCreateMemStorage(0); // *** add by memory_leak2_20141024.patch
      ...
      cvReleaseImage(&frame);   // *** add by memory_leak_20141023.patch
      cvReleaseMemStorage(&houghStorage); // *** add by memory_leak2_20141024.patch     
    }

  /* release resources */
  //  cvReleaseMemStorage(&houghStorage); // *** deleted by memory_leak2_20141024.patch

  cvReleaseImage(&gray);
  cvReleaseImage(&edges);
  cvReleaseImage(&temp_frame);
  cvReleaseImage(&half_frame);
  //  cvReleaseImage(&frame);  // *** delete by memory_leak_20141023.patch
