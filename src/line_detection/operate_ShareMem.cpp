...
static int *shrd_ptr_process_check;
...
void attach_ShareMem(void)
{
  ...
  MY_ERROR_CHECK(shm_key_process_check, "key generation", "process checker");
  ...
  MY_ERROR_CHECK(shrd_id_process_check, "Access shared memory", "process checker");
...
IplImage *getImage_fromSHM(void)
{
  ...
  cvReleaseMat(&buf);           // add by memory_leak_20141023.patch
  return (image);
}
...
void detach_ShareMem(void)
{
 ...
 MY_ERROR_CHECK(shmdt(shrd_ptr_process_check), "detach", "process checker");
