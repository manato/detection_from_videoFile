...
//    char *pathname_input, *pathname_output, *pathname_height, *pathname_width;
    ...
    /* delete fps keeper */
    shmctl(shrd_id_car_fps, IPC_RMID, 0);
    shmctl(shrd_id_pds_fps, IPC_RMID, 0);
    shmctl(shrd_id_sgn_fps, IPC_RMID, 0);

    /* delete process checker */
    shmctl(shrd_id_process_check, IPC_RMID, 0);
