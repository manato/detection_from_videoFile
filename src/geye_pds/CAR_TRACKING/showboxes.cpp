...
void show_rects(IplImage *Image,RESULT *CUR,FLOAT ratio)
{
  ...
  /* generate key for process checker */
  key_t shm_key_process_check = ftok(PROCESS_CHECK_SHM_PATH, 1);
  if (shm_key_process_check == -1) {
    printf("key generation for process checker is failed\n");
  }
  ...
  *shrd_ptr_process_check += 1;
  ...
  if(shmdt(shrd_ptr_process_check)==-1){
    printf("purge error! (shrd_ptr_process_check)\n");
  }
