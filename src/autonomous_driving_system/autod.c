...
//    char *pathname_input, *pathname_output, *pathname_height, *pathname_width;
    ...
    /* key generation for frame per second keeper */
    ...
    if (shm_key_process_check == -1) { // error semantics
        printf("generation key for process_checker failed\n");
        return;
    }
...
//    shrd_id_rbuf = shmget(shm_key_rbuf, MAX_OBJECT_NUM*sizeof(int*), IPC_CREAT|0666);  // 0666 : readable and writable for all user
    ...
    /* create shared frame per second keeper */
    shrd_id_car_fps = shmget(shm_key_car_fps, sizeof(int), IPC_CREAT|0666);
    if(shrd_id_car_fps < 0) {  // error semantics
        printf("Can't Create Shared memory for car_fps...\n");
        return;
    }

    shrd_id_pds_fps = shmget(shm_key_pds_fps, sizeof(int), IPC_CREAT|0666);
    if(shrd_id_pds_fps < 0) {  // error semantics
        printf("Can't Create Shared memory for pds_fps...\n");
        return;
    }

    shrd_id_sgn_fps = shmget(shm_key_sgn_fps, sizeof(int), IPC_CREAT|0666);
    if(shrd_id_sgn_fps < 0) {  // error semantics
        printf("Can't Create Shared memory for sgn_fps...\n");
        return;
    }

    /* create shared process checker */
    shrd_id_process_check = shmget(shm_key_process_check, sizeof(int), IPC_CREAT|0666);
    if (shrd_id_process_check < 0) { // error semantics
        printf("Can't Create Shared memory for process check...\n");
        return;
    }
...
//    if( (pthread_rwlock_init(shrd_ptr_rwlock, NULL)) != 0) {
    ...
    /* initialization of each detection program frame per second */
    memset(shrd_ptr_car_fps, 0, sizeof(int));
    memset(shrd_ptr_pds_fps, 0, sizeof(int));
    memset(shrd_ptr_sgn_fps, 0, sizeof(int));

    /* initialization of process checker */
    memset(shrd_ptr_process_check, 0, sizeof(int));
    ...
    /* get file size */
#ifdef RELEASE
    if( (src_fp = fopen("/usr/local/geye_with_cam/bin/shm_creater/lena.jpg", "rb")) == NULL)
#else
